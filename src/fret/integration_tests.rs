...
use super::{
    ChordFingeringGenerator, ChordFingeringConfig, StringedFretboard, 
    InstrumentPresets, SkillLevel,
    traits::{FingeringGenerator, Fretboard},
};
use crate::{Chord, ChordQuality, Tuning, PitchClass, Inversion};
...
mod chord_integration_tests {
    ...
    fn test_basic_chord_types_integration() {
        ...
        let fingerings = generator.generate_chord_fingerings(&fretboard, &c_major).unwrap();
        assert!(!fingerings.is_empty(), "Should generate fingerings for C major");

        // Test minor triad
        let a_minor = Chord::new(Tuning::new(PitchClass::A, 2), ChordQuality::Minor).unwrap();
        let fingerings = generator.generate_chord_fingerings(&fretboard, &a_minor).unwrap();
        assert!(!fingerings.is_empty(), "Should generate fingerings for A minor");

        // Test dominant 7th
        let g7 = Chord::new(Tuning::new(PitchClass::G, 2), ChordQuality::Dominant7).unwrap();
        let fingerings = generator.generate_chord_fingerings(&fretboard, &g7).unwrap();
        ...
        let fingerings = generator.generate_chord_fingerings(&fretboard, &cmaj7).unwrap();
        assert!(!fingerings.is_empty(), "Should generate fingerings for Cmaj7");

        // Test minor 7th
        let dm7 = Chord::new(Tuning::new(PitchClass::D, 3), ChordQuality::Minor7).unwrap();
        let fingerings = generator.generate_chord_fingerings(&fretboard, &dm7).unwrap();
    ...
    fn test_extended_chords_integration() {
        ...
        match fingerings {
            ...
            Err(_) => println!("Cadd9 not playable on standard guitar (expected for some extended chords)"),
        }
        ...
        match fingerings {
            ...
            Err(_) => println!("G9 not playable on standard guitar (expected for some extended chords)"),
        }
    }
    ...
    fn test_chord_inversions_integration() {
        ...
        // Test root position
        let c_major = Chord::new(Tuning::new(PitchClass::C, 3), ChordQuality::Major).unwrap();
        let root_fingerings = generator.generate_chord_fingerings(&fretboard, &c_major).unwrap();
        assert!(!root_fingerings.is_empty(), "Should generate fingerings for C major root position");

        // Test first inversion
        let mut c_major_first = c_major.clone();
        c_major_first.invert(Inversion::First);
        let first_fingerings = generator.generate_chord_fingerings(&fretboard, &c_major_first).unwrap();
        assert!(!first_fingerings.is_empty(), "Should generate fingerings for C major first inversion");

        // Test second inversion
        let mut c_major_second = c_major.clone();
        c_major_second.invert(Inversion::Second);
        let second_fingerings = generator.generate_chord_fingerings(&fretboard, &c_major_second).unwrap();
        assert!(!second_fingerings.is_empty(), "Should generate fingerings for C major second inversion");
        ...
        assert_ne!(root_components[0], first_components[0], "First inversion should have different bass note");
        assert_ne!(root_components[0], second_components[0], "Second inversion should have different bass note");
        assert_ne!(first_components[0], second_components[0], "Inversions should have different bass notes");
    }
    ...
    fn test_all_chord_qualities_integration() {
        ...
        for quality in &chord_qualities {
            ...
            match result {
                Ok(fingerings) => {
                    println!("Generated {} fingerings for C{:?}", fingerings.len(), quality);
                    assert!(!fingerings.is_empty(), "Should generate at least one fingering for C{:?}", quality);
                }
                Err(e) => {
                ...
                Ok(chord) => {
                    ...
                    match result {
                        Ok(fingerings) => {
                            ...
                            assert!(!fingerings.is_empty(), "Should generate fingerings for {}", symbol);
                        }
                        Err(e) => {
    ...
    fn test_cross_instrument_chord_integration() {
        ...
        for (name, config) in &instruments {
            ...
            match result {
                Ok(fingerings) => {
                    println!("Generated {} fingerings for G major on {}", fingerings.len(), name);
                    assert!(!fingerings.is_empty(), "Should generate fingerings for G major on {}", name);
                }
                Err(e) => {
                    println!("Could not generate fingerings for G major on {}: {}", name, e);
                }
            }
        }
    }
    ...
    fn test_skill_level_chord_adaptation() {
        ...
        for skill_level in &skill_levels {
            let config = ChordFingeringConfig::new()
            ...
            match result {
                Ok(fingerings) => {
                    println!("Generated {} fingerings for F major at {:?} level", 
                             fingerings.len(), skill_level);
                    ...
                    if !fingerings.is_empty() {
                        ...
                        println!("Average difficulty for {:?}: {:.2}", skill_level, avg_difficulty);
                ...
                Err(e) => {
                    println!("Could not generate fingerings for F major at {:?} level: {}", skill_level, e);
                }
            }
        }
    }
    ...
    fn test_chord_components_validation() {
        ...
        for chord in &test_chords {
            ...
            let fingerings = generator.generate_chord_fingerings(&fretboard, chord).unwrap();
            
            assert!(!fingerings.is_empty(), "Should generate fingerings for chord");
            
            // For each fingering, verify it can produce the chord tones
            for (i, fingering) in fingerings.iter().take(3).enumerate() { // Test first 3 fingerings
                ...
                let produced_pitch_classes: Vec<PitchClass> = produced_notes
                    .iter()
                    .map(|t| t.class())
                    .collect();
                ...
                let has_chord_tones = expected_pitch_classes.iter()
                    .any(|&pc| produced_pitch_classes.contains(&pc));
                
                assert!(has_chord_tones,
                       "Fingering {} should contain at least one chord tone", i);
                
                println!("Fingering {} for chord produces notes: {:?}", i, produced_notes);
            }
        }
    }
}
...
mod tuning_integration_tests {
    ...
    fn test_all_pitch_classes_integration() {
        ...
        for pitch_class in &pitch_classes {
            ...
            assert!(!positions.is_empty(), 
                   "Should find positions for {:?}3 on guitar", pitch_class);
            
            // Test that tuning_at_position works correctly
            for position in positions.iter().take(3) { // Test first 3 positions
                let found_tuning = fretboard.tuning_at_position(position).unwrap();
                // Check that the pitch class is enharmonically equivalent
                assert_eq!(found_tuning.number(), tuning.number(),
                          "Position {:?} should produce same MIDI number as {:?}3", position, pitch_class);
            }
        }
    }
    ...
    fn test_enharmonic_equivalence_integration() {
        ...
        for (sharp, flat) in &enharmonic_pairs {
            ...
            assert_eq!(sharp_tuning.number(), flat_tuning.number(),
                      "{:?} and {:?} should have same MIDI number", sharp, flat);
            ...
            assert!(!sharp_positions.is_empty(), "Should find positions for {:?}4", sharp);
            assert!(!flat_positions.is_empty(), "Should find positions for {:?}4", flat);
            
            // The positions should be the same (since they're enharmonically equivalent)
            assert_eq!(sharp_positions, flat_positions,
                      "{:?}4 and {:?}4 should have same positions", sharp, flat);
        }
    }
    ...
    fn test_octave_range_stringed(fretboard: &StringedFretboard, instrument_name: &str) {
        ...
        for octave in 1..=6 {
            ...
            if !positions.is_empty() {
                println!("{} can play A{} at {} positions", instrument_name, octave, positions.len());
                
                // Verify the positions actually produce the correct octave
                for position in positions.iter().take(2) {
                    let found_tuning = fretboard.tuning_at_position(position).unwrap();
                    assert_eq!(found_tuning.octave(), octave,
                              "Position should produce A{}, got A{}", octave, found_tuning.octave());
                }
            } else {
    ...
    fn test_octave_range_continuous(fretboard: &crate::fret::ContinuousFretboard, instrument_name: &str) {
    ...
    fn test_microtonal_tuning_support() {
        ...
        assert!(!positions_440.is_empty(), "Should find positions for standard A4");
        ...
        for octave in 0..=8 {
            ...
            if !positions.is_empty() {
                ...
                for position in positions.iter().take(2) {
                    let found_tuning = fretboard.tuning_at_position(position).unwrap();
                    assert_eq!(found_tuning.number(), test_tuning.number(),
                              "Position should produce correct MIDI number for C{}", octave);
                }
            }
        }
    }
    ...
    fn test_extended_range_compatibility() {
        ...
        if !positions.is_empty() {
            println!("5-string bass can reach G4 at {} positions", positions.len());
        }
    }
    ...
    fn test_guitar_alternative_tunings(fretboard: &StringedFretboard) {
        ...
        if positions.is_empty() {
            ...
            println!("Guitar can play low D for drop D tuning at {} positions", positions.len());
        }
        ...
        for (i, tuning) in open_g_notes.iter().enumerate() {
            let positions = fretboard.positions_for_tuning(tuning);
            if positions.is_empty() {
                println!("Note {} for open G tuning not reachable (may require retuning)", i + 1);
            } else {
    ...
    fn test_continuous_alternative_tunings(fretboard: &crate::fret::ContinuousFretboard, instrument_name: &str) {
        use crate::fret::traits::Fretboard;
        ...
        for tuning in &test_notes {
            let positions = fretboard.positions_for_tuning(*tuning);
            if !positions.is_empty() {
                println!("{} can play {} at {} positions", 
                         instrument_name, tuning, positions.len());
                ...
                assert!(!positions.is_empty(), 
                       "{} should be able to find positions for {}", instrument_name, tuning);
            }
        }
    }
}
                            ...
                            Ok(fingerings) => {
                                ...
                                for fingering in &fingerings {
                                    ...
                                    for finger_pos in &fingering.positions {
                                        ...
                                        prop_assert!(pos.string < fretboard.string_count(),
                                                   "String index {} should be within bounds (0-{})",
                                                   pos.string, fretboard.string_count() - 1);
                                        prop_assert!(pos.fret <= fretboard.fret_count(),
        ...
        proptest! {
            ...
            fn property_cross_instrument_consistency(
                ...
                for (name, config) in &instruments {
                    ...
                    for position in &positions {
                        prop_assert!(position.string < fretboard.string_count(),
                                   "{}: string {} should be valid", name, position.string);
                        prop_assert!(position.fret <= fretboard.fret_count(),
...
mod composition_integration_tests {
    use super::*;
    use crate::composition::{Measure, Track, Score};
    ...
    fn test_measure_chord_integration() {
        ...
        if let Measure::Chords(chords) = &measure {
            ...
            let fingerings = generator.generate_chord_fingerings(&fretboard, &chords[0]).unwrap();
            assert!(!fingerings.is_empty(), "Should generate fingerings for chord in measure");
        } else {
        ...
        if let Measure::Chords(chords) = &multi_chord_measure {
            ...
            for chord in chords {
                let fingerings = generator.generate_chord_fingerings(&fretboard, chord).unwrap();
                assert!(!fingerings.is_empty(), "Should generate fingerings for each chord in measure");
            }
        }
    }
            ...
            if let Measure::Chords(measure_chords) = measure {
                for chord in measure_chords {
                    ...
                    assert!(!fingerings.is_empty(), 
                           "Should generate fingerings for chord {} in track", i);
                }
            }
        }
    }
    ...
    fn test_score_multi_track_integration() {
        ...
        score.new_measures(|measures| {
            ...
            measures[0].chord(Chord::new(Tuning::new(PitchClass::C, 3), ChordQuality::Major).unwrap());
            // Track 2: A minor chord
            measures[1].chord(Chord::new(Tuning::new(PitchClass::A, 2), ChordQuality::Minor).unwrap());
        });

        score.new_measures(|measures| {
            // Track 1: F major chord
            measures[0].chord(Chord::new(Tuning::new(PitchClass::F, 3), ChordQuality::Major).unwrap());
            // Track 2: G major chord
            measures[1].chord(Chord::new(Tuning::new(PitchClass::G, 3), ChordQuality::Major).unwrap());
        });
    ...
    fn test_multi_measure_fingering_optimization() {
        ...
        for chord in &progression {
            let fingerings = generator.generate_chord_fingerings(&fretboard, chord).unwrap();
            assert!(!fingerings.is_empty(), "Should generate fingerings for chord {}", chord);
        ...
        for (i, fingerings) in all_fingerings.iter().enumerate() {
            println!("Chord {} ({}) has {} possible fingerings", 
                     i, progression[i], fingerings.len());
    ...
    fn test_composition_skill_level_adaptation() {
        ...
        score.new_measures(|measures| {
            ...
            measures[0].chord(Chord::new(Tuning::new(PitchClass::B, 3), ChordQuality::Major).unwrap());
        });

        let skill_levels = [SkillLevel::Beginner, SkillLevel::Intermediate, SkillLevel::Advanced];
                ...
                for measure in track.get_measures() {
                    if let Measure::Chords(chords) = measure {
                        for chord in chords {
                            ...
                            match result {
                                Ok(fingerings) => {
                                    println!("Generated {} fingerings for {} at {:?} level", 
                                             fingerings.len(), chord, skill_level);
                                    ...
                                    if !fingerings.is_empty() {
                                        ...
                                        if *skill_level == SkillLevel::Beginner {
                                            assert!(avg_difficulty <= 0.8, 
                                                   "Beginner fingerings should be easier");
                                        }
                                    }
                                }
                                Err(e) => {
                                    println!("Could not generate fingerings for {} at {:?} level: {}", 
                                             chord, skill_level, e);
                ...
                for (measure_idx, measure) in track.get_measures().iter().enumerate() {
                    if let Measure::Chords(chords) = measure {
                        for chord in chords {
                            ...
                            match result {
                                Ok(fingerings) => {
                                    println!("{} can play {} in measure {} ({} fingerings)", 
                                             instrument_name, chord, measure_idx, fingerings.len());
                                    assert!(!fingerings.is_empty(), 
                                           "{} should be able to play {}", instrument_name, chord);
                                }
                                Err(e) => {
                                    println!("{} cannot play {} in measure {}: {}", 
